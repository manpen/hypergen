...
Graph HyperbolicGenerator::generate(count n, double R, double alpha, double T) {
    auto points = generatePoints(n, R, alpha, T);
    return generate(points.first, points.second, R, T);
}

std::pair<std::vector<double>, std::vector<double>> HyperbolicGenerator::generatePoints(count n, double R, double alpha, double T) const {
	assert(R > 0);
